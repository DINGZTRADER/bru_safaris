...
export function Header() { const [open, setOpen] = useState(false); return <header className="header"><Link className="brand" href="/" aria-label="Bru Safaris home"><Image className="brand-logo" src="/images/bru-safaris-logo.png" alt="" width={72} height={72} priority/><span>BRU<br/><small>SAFARIS</small></span></Link><nav className="desktop-nav" aria-label="Main navigation">{links.map(([name, href]) => <Link key={href} href={href}>{name}</Link>)}<Link className="button small" href="/plan">Design my safari <span>↗</span></Link></nav><button className="menu" onClick={() => setOpen(!open)} aria-expanded={open} aria-label="Toggle menu">{open ? <X/> : <Menu/>}</button>{open && <nav className="mobile-nav" aria-label="Mobile navigation">{links.map(([name, href]) => <Link onClick={() => setOpen(false)} key={href} href={href}>{name}</Link>)}<Link onClick={() => setOpen(false)} href="/plan">Design my safari</Link></nav>}</header>; }
